...
const ITERATION_EXP_WORDS_LENGTH = parseInt((ID_BITS_LENGTH + EXTENDABLE_BACKUP_FLAG_BITS_LENGTH + ITERATION_EXP_BITS_LENGTH + RADIX_BITS - 1) / RADIX_BITS, 10);
...
const MNEMONICS_WORDS_LENGTH = parseInt(METADATA_WORDS_LENGTH + (MIN_ENTROPY_BITS + RADIX_BITS - 1) / RADIX_BITS, 10);
...
function combineMnemonics(mnemonics, passphrase = '') {
  ...
  groups.forEach((members, groupIndex) => {
    ...
    if (maybeJSBI.notEqual(shares.size, threshold)) {
      const prefix = groupPrefix(identifier, extendableBackupFlag, iterationExponent, groupIndex, groupThreshold, groupCount);
...
function groupPrefix(identifier, extendableBackupFlag, iterationExponent, groupIndex, groupThreshold, groupCount) {
  const idExpInt = JSBI.BigInt((identifier << ITERATION_EXP_BITS_LENGTH + EXTENDABLE_BACKUP_FLAG_BITS_LENGTH) + (extendableBackupFlag << ITERATION_EXP_BITS_LENGTH) + iterationExponent);
...
function encodeMnemonic(identifier, extendableBackupFlag, iterationExponent, groupIndex, groupThreshold, groupCount, memberIndex, memberThreshold, value) {
  ...
  const gp = groupPrefix(newIdentifier, extendableBackupFlag, iterationExponent, groupIndex, groupThreshold, groupCount);
