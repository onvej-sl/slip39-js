...
class Slip39 {
  ...
  static fromArray(masterSecret, {
    ...
    const encryptedMasterSecret = slipHelper.crypt(masterSecret, passphrase, iterationExponent, slip.identifier, extendableBackupFlag);
  ...
  buildRecursive(currentNode, nodes, secret, threshold, index) {
    ...
    if (nodes.length === 0) {
      ...
      const mnemonic = slipHelper.encodeMnemonic(this.identifier, this.extendableBackupFlag, this.iterationExponent, index, this.groupThreshold, this.groupCount, currentNode.index, threshold, secret);
